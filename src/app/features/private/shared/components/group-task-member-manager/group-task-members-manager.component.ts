...
export class GroupTaskMembersManagerComponent implements OnInit{
  ...
  protected addMemberToTask(memberId:string): void {
    ...
    this.groupTaskMemberApi.assignMemberToTask(this.groupId(),this.task().groupTaskId,request).subscribe({
      ...
      next: (response) => {
        ...
        ([...this.assignedMembers(),this.membersList().find
        (member=>member.groupMemberId===memberId)!
        ]);
  ...
  protected removeMemberFromTask(memberId:string): void {
    ...
    let taskMemberId = this.task().groupTaskMembers.find(
      taskMember=>taskMember.groupMemberId===memberId)?.groupTaskMemberId!;

    this.groupTaskMemberApi.removeMemberFromTask(this.groupId(),this.task().groupTaskId,taskMemberId).subscribe(() => {
      ...
      this.notAssignedMembers.set
      ([...this.notAssignedMembers(),this.membersList().find
      (member=>member.groupMemberId===memberId)!
      ]);
  ...
  ngOnInit(): void {
    this.notAssignedMembers.set(
      ...
      (member=>member.groupMemberId!=this.task().groupTaskMembers.find(
        taskMember=>taskMember.groupMemberId===member.groupMemberId)?.groupMemberId));
    this.assignedMembers.set(
      this.membersList().filter
      (member=>this.task().groupTaskMembers.find(
        taskMember=>taskMember.groupMemberId===member.groupMemberId)?.groupMemberId===member.groupMemberId));
