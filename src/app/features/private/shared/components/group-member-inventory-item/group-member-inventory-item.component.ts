...
import {OwnedGroupItemModel, OwnedGroupItemRequestModel} from '../../../../shared/models/group/owned-group-item.model';
...
export class GroupMemberInventoryItemComponent
{
  ...
  useItem()
  {
   const request:OwnedGroupItemRequestModel={isUsedUp:true}
    this.inventoryApi.useMemberItem(this.groupId(),this.memberId(),this.item().id,request)
  ...
  deleteItem()
  {
    this.inventoryApi.deleteMemberInventoryItem(this.groupId(),this.memberId(),this.item().id)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe(
        {
          next:()=>{
            this.notificationApi.success("Successfully deleted item from inventory")
            this.actionPerformed.emit()
          },
          error:()=>{
            this.notificationApi.error("Error occurred during inventory item delete")
          }
        }
      )
  }
